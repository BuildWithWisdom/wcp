...
class SimulationService {
     ...
     */
    simulateMatch(homeTeam, awayTeam, isKnockout, modifiers) {
     ...
     */
    generateTimeline(homeTeam, awayTeam, homeGoals, awayGoals, startMin, endMin) {
        const events = [];
        const addGoalEvents = (team, count) => {
     ...
     */
    simulatePenalties(homeTeam, awayTeam) {
        const events = [];
        let homePenState = 0;
        let awayPenState = 0;
        let homeScored = 0;
        ...
        const simulateKick = (rate) => Math.random() < rate;
        for (let round = 1; round <= 5; round++) {
            ...
            if (homeScored > awayScored + (5 - awayPenState))
                break;
            if (awayScored > homeScored + (5 - homePenState))
                break;
     ...
     */
    generateWittySummary(home, away, result) {
